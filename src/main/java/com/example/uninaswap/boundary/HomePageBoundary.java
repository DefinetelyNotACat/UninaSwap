...
public class HomePageBoundary implements GestoreMessaggio {
    ...
    private VBox creaCardUtente(Utente utente) {
        ...
        btnProfilo.setOnAction(e -> {
            ...
            if (annunciUtente.isEmpty()) mostraMessaggioVuoto("Nessun annuncio.", utente.getUsername() + " non ha annunci.");
        ...
        btnRecensioni.setOnAction(e -> {
            try {
                ...
                controllerRecensioni.initData(utente);
    ...
    private VBox creaCardAnnuncio(Annuncio annuncio) {
        ...
        caricaFotoOggetto(annuncio, imgView);

        HBox headerInfo = new HBox(10);
        headerInfo.setAlignment(Pos.CENTER_LEFT);
        Label badge = new Label(determinaTipo(annuncio));
        badge.getStyleClass().addAll("badge-base", determinaClasseBadge(annuncio));
        Text location = new Text("📍 " + (annuncio.getSede() != null ? annuncio.getSede().getNomeSede() : "N/A"));
        ...
        if (annuncio instanceof AnnuncioVendita av) extraInfo.setText("💰 " + av.getPrezzoMedio() + " €");
        else if (annuncio instanceof AnnuncioScambio as) extraInfo.setText("🔄 " + as.getListaOggetti());
        ...
        card.setOnMouseClicked(e -> apriDettaglioAnnuncio(annuncio));
        return card;
    }
    ...
    private void caricaFotoOggetto(Annuncio annuncio, ImageView imageView) {
        try {
            if (annuncio.getOggetti() != null && !annuncio.getOggetti().isEmpty() && !annuncio.getOggetti().get(0).getImmagini().isEmpty()) {
                File f = new File(System.getProperty("user.dir") + File.separator + "dati_utenti" + File.separator + annuncio.getOggetti().get(0).getImmagini().get(0));
                if (f.exists()) {
                    imageView.setImage(new Image(f.toURI().toString(), 400, 300, true, true));
                    return;
                }
            }
