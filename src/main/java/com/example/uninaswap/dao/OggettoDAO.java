        ...
        try {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sqlOggetto, Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (ResultSet rs = stmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        ...
                        oggetto.setId(idOggettoGenerato); // ORA l'oggetto ha l'ID!
                    }
                }
            }

            // --- GESTIONE FISICA DELLE IMMAGINI ---
            if (oggetto.getImmagini() != null && !oggetto.getImmagini().isEmpty()) {
                ...
                for (String pathTemporaneo : oggetto.getImmagini()) {
                    // Chiamiamo il metodo nell'Entity che crea la cartella e copia il file
                    // Questo metodo ora userà l'ID appena generato
                    String pathRelativo = oggetto.copiaImmagineInLocale(pathTemporaneo);
                    if (pathRelativo != null) {
                        percorsiDefinitivi.add(pathRelativo);
                    }
                }

                // Aggiorniamo l'oggetto con i nuovi path relativi (es. oggetti/12/foto.jpg)
                oggetto.setImmagini(percorsiDefinitivi);

                // Salviamo i path relativi nella tabella IMMAGINE
            ...
            // Se qualcosa fallisce (anche la copia del file), annulliamo tutto nel DB
